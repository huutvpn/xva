...
VALIDITY () {
    ...
    Exp1=$(curl https://raw.githubusercontent.com/huutvpn/ip/main/ip | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://github.com/huutvpn/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# validity
rm -f /usr/bin/e
valid=$(curl https://github.com/huutvpn/ip | grep $MYIP | awk '{print $4}')
...
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(curl https://github.com/huutvpn/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://github.com/huutvpn/xva/tree/main/"
